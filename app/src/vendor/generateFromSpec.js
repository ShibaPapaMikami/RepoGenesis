...
- Owner: ${(0,N.formatOwner)(o.owner)}
${i}## Tech Stack
- Domains: ${(0,N.formatDomains)(e.tech.domains)}
...
${ao(e,o)}
`}if(n==="workspace"){let i=e.structure.repos.map(m=>{let p=m.depends_on.length>0?` (depends on: ${m.depends_on.join(", ")})`:"";return`- ${m.name}: ${m.description}${p}`}).join(`
`);return`# ${e.project.name} \u2014 Workspace Constitution

## What is this workspace?
${(0,N.formatProjectDescription)(e.project.description)}
...
## Repository Structure
${so(e)}
`}});var H=d(ae=>{"use strict";Object.defineProperty(ae,"__esModule",{value:!0});ae.generateToolGuidance=ho;var W=$(),po=O(),uo={codex:"Codex",claude_code:"Claude Code",gemini_cli:"Gemini CLI"},mo={codex:"- Prefer repository-local Codex skills or guidance artifacts when they exist.",claude_code:"- Prefer repository-local Claude Code skills when they exist.",gemini_cli:"- Prefer repository-local Gemini commands or context artifacts when they exist."};function ho(e,t,n={}){let o=n.scope??"single",r=uo[t],s=(0,po.getToolWrapperFile)(t),a=mo[t],l=(0,W.inferBriefSignals)(e),i=(0,W.hasOperatorFacingUi)(e),m=(0,W.hasStableCliSurface)(e),c=(0,W.summarizeSupportingLanguages)(e).includes("python"),g=e.planning?.tech_decisions??[],_=e.tech.frameworks.some(u=>/tauri/i.test(u))||g.some(u=>u.status==="adopted"&&/framework/i.test(u.topic)&&/tauri/i.test(u.choice)),E=e.tech.frameworks.some(u=>/typer/i.test(u))||g.some(u=>u.status==="adopted"&&/framework/i.test(u.topic)&&/typer/i.test(u.choice)),k=g.some(u=>u.status==="adopted"&&/inference bridge/i.test(u.topic)&&u.choice.trim()),I=g.some(u=>u.status==="adopted"&&/sidecar packaging/i.test(u.topic)&&u.choice.trim()),h=[m?"- Treat the CLI contract as first-class: keep command examples, flags, exit behavior, and output locations explicit.":null,m&&e.tech.primary_language==="python"&&E?"- For Python CLI projects using `Typer`, keep command groups, help text, option types, and generated CLI help aligned with the documented contract.":null,m&&e.tech.primary_language==="python"&&!E?"- For Python CLI projects, prefer `pyproject.toml` and default to `argparse` unless a richer subcommand tree is clearly justified.":null,l.hasPipeline?"- Keep the first processing pipeline explicit end to end, including stage inputs, outputs, and tunable parameters that affect results.":null,l.hasTts?"- When synthesis or media quality depends on parameters such as voice, speed, pitch, breath, or break, document their defaults and intended effect next to the implementation.":null,l.hasTranscription?"- For transcription products, keep capture source, timestamp or speaker-label behavior, transcript storage format, and export contract explicit from the first release.":null,i&&_&&!c?"- If the operator UI uses a desktop shell, keep app packaging, local file boundaries, and runtime prerequisites explicit from the start.":null,i&&(c||k||I)?"- If the operator UI uses a desktop shell or local sidecar runtime, keep the shell-to-sidecar bridge, packaging method, and local artifact handoff explicit from the start.":null,l.hasUnity?"- Keep the Unity integration boundary explicit: define handoff artifacts, expected file formats, and runtime assumptions before coding across the boundary.":null,i?"- For operator-facing UI, keep a small low-emphasis runtime label in the top-right header showing release version, commit, and deploy or publication time during active development and rollout.":null,i?"- Implement the runtime label so it can later be hidden, feature-flagged, or restricted to admins without removing API/log-based traceability.":null].filter(Boolean),C=h.length>0?`${h.join(`
`)}
...
`}});var mt=d(he=>{"use strict";Object.defineProperty(he,"__esModule",{value:!0});he.generateTechDecisions=Ao;var ut=b();function X(e,t,n,o){let r=(0,ut.getTechDecisionsByStatus)(e,t);return r.length===0?`## ${n}
${o}`:`## ${n}
${r.map(s=>`### ${s.topic}
- **Choice**: ${s.choice}
...
`}});var ht=d(fe=>{"use strict";Object.defineProperty(fe,"__esModule",{value:!0});fe.generateExternalDependencies=So;var ge=b();function z(e,t,n,o){let r=(0,ge.getDependenciesByStatus)(e,t);return r.length===0?`## ${n}
${o}`:`## ${n}
${r.map(s=>`### ${s.name}
...
`}});var gt=d(ye=>{"use strict";Object.defineProperty(ye,"__esModule",{value:!0});ye.generateRequirements=Io;var bo=b(),P=$(),J=D();function Io(e){let{project:t,tech:n,security:o,structure:r}=e,s=e.planning??{tech_decisions:[],external_dependencies:[]},a=(0,P.inferBriefSignals)(e),l=(0,P.inferPipelineStages)(e),i=(0,P.summarizeCoreFeatures)(e),m=(0,P.summarizeDependencyNames)(e,"adopted"),p=(0,P.summarizeRuntimeBoundary)(e),c=(0,P.summarizeSupportingLanguages)(e),g=(0,P.summarizeTranscriptionContract)(e),_=(0,P.hasOperatorFacingUi)(e),E=(0,P.hasStableCliSurface)(e),k=n.domains.includes("web")||n.frameworks.length>0||s.tech_decisions.some(f=>/framework/i.test(f.topic)),I=n.frameworks.length>0||s.tech_decisions.some(f=>/framework/i.test(f.topic)&&f.choice.trim()),h=n.frameworks.length>0?`- Frameworks: ${n.frameworks.join(", ")}
`:"",C=c.length>0?`- Supporting Languages: ${c.join(", ")}
`:"",u=[{title:"R1: Deliver the primary workflow",description:`${t.name} must support the first useful user outcome described in the overview: ${(0,J.formatProjectDescription)(t.description)}.`,criteria:[`A user can complete the first end-to-end workflow for ${t.name}.`,"The main inputs and outputs for that workflow are explicitly handled in code or documented in the repository.","The first workflow is small enough to deliver within the current planning horizon without broadening scope unnecessarily.","The exact boundary of the initial scope is written down, including what is included now and what is explicitly deferred."]},{title:"R2: Keep the project operable and traceable from day one",description:`${t.name} must remain easy to start, safe to configure, and easy to inspect while the product scope is still evolving.`,criteria:["Local setup expectations and required environment placeholders are documented.",`Security expectations for level \`${o.level}\` are reflected in implementation and deployment decisions.`,"Release version and commit identity can be surfaced by the running service, API, or CLI when applicable.",..._?["Operator-facing UI keeps a low-emphasis runtime label in the top-right header showing release version, commit SHA, and deploy or publication time during active development and rollout.","The UI label can later be hidden, feature-flagged, or restricted to admins without removing other runtime identity surfaces."]:[]]}];if(r.repo_type==="multi"){let f=r.repos.map(T=>T.name).join(", ");u.push({title:`R${u.length+1}: Keep repository boundaries explicit`,description:`The workspace must keep responsibilities clear across the initial repositories: ${f}.`,criteria:["Each repository has a clearly named responsibility and owner.","Cross-repository dependencies are documented before implementation work starts.","Shared decisions stay in workspace-level docs and do not drift into repo-local copies."]})}if(a.hasPipeline){let T=[l.length>0?`The initial stage order is explicit: ${l.join(" -> ")}.`:"The initial stage order is explicit and documented before implementation expands.","Inputs, outputs, and failure boundaries for each stage are documented in code, tests, or repository docs.","The output contract for the first workflow is specified, including response, file, or artifact format when applicable."];a.hasTunableParameters&&T.push("Tunable parameters that materially affect generated output are listed with defaults and intended effects."),a.hasTts&&T.push("Audio-related parameters and output format requirements are documented when they affect quality or compatibility."),a.hasTranscriptionControls&&(T.push("Transcription-related controls and output format requirements are documented when they affect review quality, timestamps, speaker markers, or downstream compatibility."),g.segmentation&&T.push(`Transcript segmentation is defined for the first workflow: ${g.segmentation}.`),g.canonicalFormat&&T.push(`A canonical transcript artifact format is defined before exports fan out: ${g.canonicalFormat}.`),g.exportFormat&&T.push(`Review and export targets are named explicitly for the first release: ${g.exportFormat}.`),g.autosave&&T.push(`Autosave behavior is defined for operator sessions: ${g.autosave}.`)),u.push({title:`R${u.length+1}: Keep the processing pipeline explicit and testable`,description:`${t.name} must describe and validate the ordered processing stages needed for the first useful output.`,criteria:T})}i.length>0&&u.push({title:`R${u.length+1}: Preserve the differentiating workflow features`,description:`${t.name} must keep the project-specific features that make the first workflow valuable explicit from the first release.`,criteria:[`The first release preserves these differentiating features: ${i.join(", ")}.`,"Each feature is mapped to code, configuration, or acceptance checks rather than being left as a generic quality goal.","Feature-specific behavior is documented close to the first workflow so implementation and review use the same language."]}),p.length>0&&u.push({title:`R${u.length+1}: Keep the client-host runtime boundary explicit`,description:`${t.name} must keep the operator-facing client and the inference or media-processing host aligned as separate but coordinated runtime responsibilities.`,criteria:[...p,"The runtime transport between client and host is named before implementation expands across machines.","Host OS, GPU, and runtime prerequisites are documented close to setup and deployment notes."]}),E&&u.push({title:`R${u.length+1}: Provide a stable operator-facing CLI contract`,description:`${t.name} must be runnable as a documented command-line workflow from the first release.`,criteria:["The primary command entrypoint and invocation examples are documented.","Arguments or options that materially change behavior are documented with expected inputs.","Exit behavior and output location or stdout/stderr contract are defined for the first workflow.","Help and version surfaces exist or are explicitly planned before release."]}),m.length>0&&u.push({title:`R${u.length+1}: Integrate adopted external dependencies intentionally`,description:`The first workflow depends on adopted external dependencies that must be introduced deliberately: ${m.join(", ")}.`,criteria:["Each adopted dependency required for the first workflow is named and mapped to a clear purpose.","License or usage terms are reviewed for adopted dependencies before release.","Required environment variables and setup prerequisites are documented.","Dependencies that send data externally have documented outbound-data expectations."]});let K=u.flatMap((f,T)=>[T===0?`### ${f.title}`:"",T===0?`- Description: ${f.description}`:`### ${f.title}`,T===0?"- Acceptance Criteria:":`- Description: ${f.description}`,...T===0?f.criteria.map(j=>`  - [ ] ${j}`):["- Acceptance Criteria:",...f.criteria.map(j=>`  - [ ] ${j}`)]]).filter(Boolean),Y=s.tech_decisions.some(f=>f.status==="open"&&/operator interface/i.test(f.topic)),B=[t.owner.trim()?null:"- Project owner is still TBD.",n.domains.length>0?null:"- Technical domain is still TBD.",k&&!I?"- Framework choice is still TBD.":null,Y&&!E?"- Operator interface is still open; do not treat CLI as the default delivery surface yet.":null,...s.tech_decisions.filter(f=>f.status==="open"&&f.topic.trim()).slice(0,5).map(f=>`- Open decision: ${f.topic}${f.choice?` -> ${f.choice}`:""}.`),...(0,bo.getDependenciesByStatus)(e,"open").slice(0,5).map(f=>`- Open dependency: ${f.name} (${f.category}).`)].filter(Boolean);return`# REQUIREMENTS.md \u2014 Functional Requirements
...
${h}${C}- AI Tooling Policy: \`docs/AI_TOOLING.md\`
...
`}});var yt=d(Ee=>{"use strict";Object.defineProperty(Ee,"__esModule",{value:!0});Ee.generateArchitecture=Co;var ft=b(),M=$(),Te=D();function Co(e){let{project:t,tech:n,structure:o}=e,r=e.planning??{tech_decisions:[],external_dependencies:[]},s=(0,ft.getAdoptedTechBulletLines)(e),a=(0,ft.getAdoptedDependencyBulletLines)(e),l=(0,M.inferPipelineStages)(e),i=(0,M.summarizeCoreFeatures)(e),m=(0,M.summarizeRuntimeBoundary)(e),p=(0,M.summarizeSupportingLanguages)(e),c=(0,M.summarizeTranscriptionContract)(e),g=n.frameworks.some(y=>/tauri/i.test(y))||r.tech_decisions.some(y=>y.status==="adopted"&&/framework/i.test(y.topic)&&/tauri/i.test(y.choice)),_=p.includes("python"),E=r.tech_decisions.find(y=>y.status==="adopted"&&/inference bridge/i.test(y.topic)&&y.choice.trim())?.choice,k=r.tech_decisions.find(y=>y.status==="adopted"&&/sidecar packaging/i.test(y.topic)&&y.choice.trim())?.choice,I=n.frameworks.length>0?`- Frameworks: ${n.frameworks.join(", ")}
`:"",h=p.length>0?`- Supporting Languages: ${p.join(", ")}
`:"",C;if(o.repo_type==="single")C=`## Repository Structure
Single repository: \`${t.slug}\``;else{let y=o.repos.map(w=>{let ln=w.depends_on.length>0?` (depends on: ${w.depends_on.join(", ")})`:"";return`- **${w.name}** (${w.type}): ${w.description}${ln} \u2014 Owner: ${(0,Te.formatOwner)(w.owner)}`}).join(`
`);C=`## Repository Structure
Multi-repository workspace: \`${t.slug}\`

### Repositories
${y}`}let u=o.repo_type==="single"?`${t.name} starts as a single-repository project focused on the first usable workflow. The architecture should keep product logic, planning docs, security rules, and release traceability close together until the system proves it needs further separation.`:`${t.name} starts as a multi-repository workspace so each major responsibility can evolve with a clear boundary. Workspace-level docs define shared rules, while repository-level docs define local architecture and execution details.`,K=i.length>0?` The first release should preserve these differentiating features explicitly: ${i.join(", ")}.`:"",Y=o.repo_type==="single"?[`- **Core product workflow**: the main implementation for ${t.name}, built in \`${n.primary_language}\` and expanded from the generated starter repository.`,...g?["- **Desktop shell**: a Tauri v2 shell hosts the operator-facing local desktop UI and app packaging boundary."]:[],..._?["- **Inference / sidecar runtime**: Python handles local ASR or media-heavy execution that should stay isolated from the UI shell."]:[],...E?[`- **Bridge layer**: ${E} keeps audio capture, inference, and transcript artifact handoff explicit between the UI shell and sidecar.`]:[],...k?[`- **Distribution packaging**: ${k} is part of the first packaging story for the sidecar or local runtime.`]:[],...i.map(y=>`- **Differentiating feature**: ${y} stays explicit in the first workflow, not implicit in generic quality language.`),"- **Documentation and planning layer**: `PROJECT.md`, `docs/REQUIREMENTS.md`, `docs/ACTIVE_CONTEXT.md`, and `docs/ROADMAP.md` hold current truth and execution context.","- **Security and configuration layer**: `SECURITY.md` and `.env.example` define setup boundaries and secret-handling expectations.","- **Version traceability layer**: `docs/VERSIONING_STANDARD.md` and `.repogenesis/manifest.json` define how release and commit identity should be exposed."].join(`
`):["- **Workspace governance layer**: `PROJECT.md`, `GLOBAL_CONTEXT.md`, `REQUIREMENTS.md`, and `SECURITY.md` define shared rules.",...o.repos.map(y=>`- **${y.name}**: ${y.description} \u2014 Owner: ${(0,Te.formatOwner)(y.owner)}.`),"- **Version traceability layer**: workspace and repository outputs should expose release and commit identity consistently."].join(`
`),B=o.repo_type==="single"?l.length>0?l.map((y,w)=>w===0?`1. The first workflow starts with \`${y}\`.`:w===l.length-1?`${w+1}. The system emits the first usable result through \`${y}\`.`:`${w+1}. The workflow advances through \`${y}\` before moving to the next stage.`).concat(E&&(g||_)?[`${l.length+1}. The UI shell and sidecar exchange artifacts through \`${E}\` before save or export completes.`]:[]).join(`
...
`),T=[c.segmentation?`- Segmentation: ${c.segmentation}`:null,c.canonicalFormat?`- Canonical format: ${c.canonicalFormat}`:null,c.exportFormat?`- Review / export targets: ${c.exportFormat}`:null,c.autosave?`- Autosave: ${c.autosave}`:null].filter(Boolean),j=m.length>0?m.map(y=>`- ${y}`).join(`
`):[g?"- The operator UI runs inside a local Tauri v2 desktop shell.":null,_?"- Media-heavy or ASR work runs in a local Python sidecar rather than inside the UI shell.":null,E?`- The shell-to-sidecar handoff is explicit through ${E}.`:null,k?`- Sidecar distribution is expected to use ${k}.`:null,!g&&!_?"- The first release currently assumes a single runtime boundary unless later planning says otherwise.":null].filter(Boolean).join(`
...
${l.length>0?l.map((y,w)=>`${w+1}. ${y}`).join(`
`):"- The first workflow shape has not been made explicit yet."}

${T.length>0?`## Transcript Contract
${T.join(`
`)}
...
`}});var _t=d(ve=>{"use strict";Object.defineProperty(ve,"__esModule",{value:!0});ve.generateRestart=jo;var No=O();function jo(e,t={}){let n=t.scope??(e.structure.repo_type==="multi"?"workspace":"single"),o=n==="repo"?"../docs/AI_TOOLING.md":"docs/AI_TOOLING.md",r=n==="repo"?"../GLOBAL_CONTEXT.md":"GLOBAL_CONTEXT.md";return`# Session Restart Protocol
...
${o}${r}
`}});var vt=d(Ae=>{"use strict";Object.defineProperty(Ae,"__esModule",{value:!0});Ae.generateEnvExample=Mo;var Lo=b();function Mo(e){let{security:t}=e,n=(0,Lo.getAdoptedEnvVars)(e),o=`# Environment Variables
...
nda/
*.confidential.*`),o+=`
`,o}});var bt=d(be=>{"use strict";Object.defineProperty(be,"__esModule",{value:!0});be.generateGlobalContext=Go;var Bo=O(),At=b(),St=D();function Go(e){let{project:t,structure:n}=e,o=(0,At.getAdoptedTechSummaryLines)(e),r=(0,At.getAdoptedDependencySummaryLines)(e),s=n.repos.map(p=>{let c=p.depends_on.length>0?` \u2192 depends on: ${p.depends_on.join(", ")}`:"";return`- **${p.name}** (${p.type}): ${p.description} \u2014 Owner: ${(0,St.formatOwner)(p.owner)}${c}`}).join(`
...
`)).join(`
`)}
\`\`\``);let i=(0,Bo.formatToolWrapperFiles)(e.tech),m=i?`- Tool-specific wrappers such as ${i} are thin adapters only.`:"- Tool-specific wrappers are thin adapters only.";return`# GLOBAL_CONTEXT.md \u2014 Multi-Repository Workspace
...
## \u8FFD\u52A0\u60C5\u5831
<!-- \u53C2\u8003\u30EA\u30F3\u30AF\u3001\u30B9\u30AF\u30EA\u30FC\u30F3\u30B7\u30E7\u30C3\u30C8\u7B49\u3002 -->
`}});var $t=d($e=>{"use strict";Object.defineProperty($e,"__esModule",{value:!0});$e.generateVersioningStandard=Xo;var Ho=$();function Xo(e){let t=(0,Ho.hasOperatorFacingUi)(e)?`### 6. Preferred operator UI label
...
${s}
`}});var Ft=d(A=>{"use strict";Object.defineProperty(A,"__esModule",{value:!0});A.DEFAULT_RUNBOOK_PATHS=void 0;A.buildDefaultRunbookEntries=yr;var lr=xt(),pr=Lt(),ur=Mt(),mr=Ut(),hr=Bt(),gr=Gt(),fr=qt();A.DEFAULT_RUNBOOK_PATHS=["docs/runbooks/README.md","docs/runbooks/production-bootstrap.md","docs/runbooks/production-cutover.md","docs/runbooks/production-checks.md","docs/runbooks/rollback.md","docs/runbooks/incident-response.md","docs/runbooks/skill-install.md"];function yr(e,t,n){return[[A.DEFAULT_RUNBOOK_PATHS[0],(0,lr.generateRunbookReadme)(e)],[A.DEFAULT_RUNBOOK_PATHS[1],(0,pr.generateProductionBootstrapRunbook)(e)],[A.DEFAULT_RUNBOOK_PATHS[2],(0,ur.generateProductionCutoverRunbook)(e)],[A.DEFAULT_RUNBOOK_PATHS[3],(0,mr.generateProductionChecksRunbook)(e)],[A.DEFAULT_RUNBOOK_PATHS[4],(0,hr.generateRollbackRunbook)(e)],[A.DEFAULT_RUNBOOK_PATHS[5],(0,gr.generateIncidentResponseRunbook)(e)],[A.DEFAULT_RUNBOOK_PATHS[6],(0,fr.generateSkillInstallRunbook)(e,t,n)]]}});var Nr=d(Je=>{Object.defineProperty(Je,"__esModule",{value:!0});Je.generateFromSpec=Dr;var Vt=O(),We=st(),Tr=it(),Er=at(),Rr=ct(),_r=lt(),Wt=pt(),Ht=mt(),Xt=ht(),zt=gt(),wr=yt(),Jt=Tt(),Kt=Et(),Yt=Rt(),Qt=_t(),Zt=wt(),en=vt(),He=kt(),vr=bt(),tn=It(),nn=Ct(),on=Ot(),rn=Pt(),Xe=$t(),sn=Dt(),an=Nt(),cn=jt(),kr=D(),dn=Ft(),Ar="1.0";function Sr(e,t){return"specVersion"in e?e.specVersion:t?.specVersion??Ar}function br(e){if("specVersion"in e){let{specVersion:t,...n}=e;return n}return e}function Ir(e,t,n,o,r){return{specVersion:o,generatorVersion:n?.generatorVersion??"dev",generatedAt:n?.generatedAt??new Date().toISOString(),source:n?.source??r,projectSlug:e.project.slug,repoType:e.structure.repo_type,fileCount:t,selectedSkills:n?.selectedSkills??[]}}function ze(e,t){let n=t.prefix??"",o=[];for(let r of(0,Vt.normalizeAiTools)(e.tech))if(r!=="other"){if(r==="codex"){o.push([`${n}AGENTS.md`,(0,Tr.generateAgentsMd)(e,t)]);continue}if(r==="claude_code"){o.push([`${n}CLAUDE.md`,(0,Er.generateClaudeMd)(e,t)]);continue}o.push([`${n}GEMINI.md`,(0,Rr.generateGeminiMd)(e,t)])}return o}function Cr(e,t){let n=new Date().toISOString().split("T")[0],o=t.depends_on.length>0?`- Depends on: ${t.depends_on.join(", ")}`:"- No dependencies",r=["`PROJECT.md`",...(0,Vt.getToolWrapperFiles)(e.tech).map(s=>`\`${s}\``)].filter(Boolean).join(`
