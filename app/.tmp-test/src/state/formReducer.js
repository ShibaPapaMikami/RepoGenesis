...
const SECURITY_ORDER = ['low', 'medium', 'high'];
function resolveSecurityLevel(minLevel, override) {
...
function recalcSecurityLevel(state) {
...
export function formReducer(state, action) {
    switch (action.type) {
        ...
        case 'SET_REPO_NAME': {
            const repos = state.structure.repos.map((r, i) => i === action.payload.index ? { ...r, name: action.payload.value } : r);
            return { ...state, structure: { ...state.structure, repos } };
        }
        case 'SET_REPO_KIND': {
            const repos = state.structure.repos.map((r, i) => i === action.payload.index ? { ...r, type: action.payload.value } : r);
            return { ...state, structure: { ...state.structure, repos } };
        }
        case 'SET_REPO_DESCRIPTION': {
            const repos = state.structure.repos.map((r, i) => i === action.payload.index ? { ...r, description: action.payload.value } : r);
            return { ...state, structure: { ...state.structure, repos } };
        }
        case 'SET_REPO_OWNER': {
            const repos = state.structure.repos.map((r, i) => i === action.payload.index ? { ...r, owner: action.payload.value } : r);
            return { ...state, structure: { ...state.structure, repos } };
        }
        case 'SET_REPO_DEPENDS_ON': {
            const repos = state.structure.repos.map((r, i) => i === action.payload.index ? { ...r, depends_on: action.payload.value } : r);
            return { ...state, structure: { ...state.structure, repos } };
        }
        // Workflow
